...
data0=rand(N*Nfft,1)-0.5;%初始化，随机数种子为66
for i=1:N*Nfft
    if data0(i)>0
    data0(i)=Pilot;
    else
...
%图案A
FdataA=data0;
FdataA(:,1:T_interA:end)=Pilot;
%图案C
FdataC=FdataA;
FdataC(1:F_inter:end,1:T_interC:end)=Pilot;%导频设置为1+1i
FdataA=FdataC;
...
%     'DopplerSpectrum',doppler('Jakes'));
 ...
 RayleighMultiPath =comm.RayleighChannel(...
    'SampleRate',sample_rate, ...
    'PathDelays',[0 10 20]/sample_rate, ...
    'AveragePathGains',[0 -4 -5], ...
    'NormalizePathGains',true, ...
